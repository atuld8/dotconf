...
# equery cft_convex | grep -e "^[0-9]\{7\}" > query_result_1
...

{
    ...
    if (Type !~ /TEST_CASE/) {
        ...
        if (state !~ /CLOSED/ && state !~ /UNASSIGNED/) Total[assigned_to]++; 
        Release[target_ver]++;
        ...
        if (state ~ /CLOSED/)       { closed_tv[target_ver]++; }
        if (state ~ /UNASSIGNED/)   { unassigned_tv[target_ver]++; }

        if (state !~ /CLOSED/ && state !~ /UNASSIGNED/) {
            # this is type per user
            if ( Type ~ /DEFECT/) defect[assigned_to]++;
            if ( Type ~ /ENHANCEMENT/) enhanc[assigned_to]++;
            if ( Type ~ /SERVICE_REQUEST/) sr[assigned_to]++;
        }
        ...
        if ( Type ~ /SERVICE_REQUEST/) {
                ...
                if (state ~ /CLOSED/)       {  closed_sr[version]++; }
        }
        
        if (state ~ /VERIFYING/ && Type ~ /SERVICE_REQUEST/)
           sr_verifying_count++; 

        if (target_ver ~ /PRIMARY/ && state !~ /CLOSED/)
...
#for (assigned_to in Total) {
    ...
    printf color_format(col_report_name,"\n\n\nReport %d: %s\n"), 2, "Etracks assinged to User and its Type"
    printf "%s\n", "---------------------------------------------------------------------------------------------------------"
    ...
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    printf "|  %-15s|        %10s        %10s        %10s        %10s        %10s        %10s        %10s        %10s        |%10s  |\n", "TARGET_VER", "OPEN", "FIXED", "WAITING", "WORKING", "VERIFYING", "FAILED", "CLOSED", "UNASSIGNED", "ACTIVE / TOTAL"
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
#for (assigned_to in Total) {
    for (i = 1; i <= index_count; i++) {
        target_ver = indices[i];
        printf "|  %-15s|        %10d        %10d        %10d        %10d        %10d        %10d        %10d        %10d        |%6d / %-5d  |\n", target_ver, open_tv[target_ver], fixed_tv[target_ver], waiting_tv[target_ver], working_tv[target_ver], ver_tv[target_ver], failed_tv[target_ver], closed_tv[target_ver], unassigned_tv[target_ver], Release[target_ver] - closed_tv[target_ver] - unassigned_tv[target_ver], Release[target_ver] 
        ...
        closed_tv_count += closed_tv[target_ver];
        unassigned_tv_count += unassigned_tv[target_ver];
        total_tv_count += Release[target_ver];
    }
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
        printf "|  %-15s|        %10d        %10d        %10d        %10d        %10d        %10d        %10d        %10d        |%6d / %-5d  |\n", "TOTAL", open_tv_count, fixed_tv_count, waiting_tv_count, working_tv_count, verifying_tv_count, failed_tv_count, closed_tv_count, unassigned_tv_count, total_tv_count - closed_tv_count - unassigned_tv_count, total_tv_count
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    ...
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------"
    printf "|  %-15s|        %10s        %10s        %10s        %10s        %10s        %10s        |%10s  |\n", "VERSION", "OPEN", "FIXED", "WAITING", "WORKING", "VERIFYING", "CLOSED", "ACTIVE / TOTAL"
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------"
    for (i = 1; i <= index_count; i++) {
        version = indices[i];
        printf "|  %-15s|        %10d        %10d        %10d        %10d        %10d        %10d        |%6d / %-5d  |\n", version, open_sr[version], fixed_sr[version], waiting_sr[version], working_sr[version], ver_sr[version], closed_sr[version], SR_ET[version] - closed_sr[version], SR_ET[version] 
        ...
        closed_sr_count += closed_sr[version];
        total_sr_count += SR_ET[version];
    }
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------"
        printf "|  %-15s|        %10d        %10d        %10d        %10d        %10d        %10d        |%6d / %-5d  |\n", "TOTAL", open_sr_count, fixed_sr_count, waiting_sr_count, working_sr_count, verifying_sr_count, closed_sr_count, total_sr_count - closed_sr_count, total_sr_count
    printf "%s\n", "---------------------------------------------------------------------------------------------------------------------------------------------------------"



    # Report 5: Summary Report
    printf color_format(col_report_name,"\n\n\nReport %d: %s\n"), 5, "Etracks Summary Report"
    printf "%s\n", "----------------------------------------------------------------------------------------------------------------------------"
    printf "|  %-30s   %5d                %-75s  |\n","Unassigned ETs", unassigned_tv_count, color_format("Yellow","All Unassigned Defect/SR/Enhancement")
    printf "|  %-30s + %5d                %-75s  |\n","Working ETs", total_count, color_format("Yellow","All Open/fixed/waiting/Working/Verifying Defect/SR/Enhancement")
    printf "|  %-30s + %5d                %-75s  |\n","Closed ETs", closed_tv_count, color_format("Yellow","All Closed Defect/SR/Enhancement")
    printf "%s\n", "----------------------------------------------------------------------------------------------------------------------------"
    printf "|  %-30s = %5d                %-75s  |\n","TOTAL ETs", total_tv_count, color_format("Yellow","Etracks under team Catagory without TEST_CASE")
    printf "|  %-30s + %5d                %-75s  |\n","TestCase ETs", TestCase_count, color_format("Yellow","All TEST_CASE type Etracks")
    printf "%s\n", "----------------------------------------------------------------------------------------------------------------------------"
    printf "|  %-30s = %5d                %-75s  |\n","TOTAL ETs", total_tv_count + TestCase_count, color_format("Yellow","Etracks under team Catagory")
    printf "%s\n", "----------------------------------------------------------------------------------------------------------------------------"

    if (unassigned_tv_count + sr_verifying_count + primary_active_count > 0)
        printf color_format("BRed","\n\n*** Note(s):\n")

    if (unassigned_tv_count > 0)
        printf color_format("Red","\tThis team has %d Etracks in UNASSIGNED state\n"), unassigned_tv_count
    if (sr_verifying_count > 0)
        printf color_format("Green","\tThis team has %d Service Request Etracks in VERIFYING state\n"), sr_verifying_count
    if (primary_active_count > 0)
        printf color_format("Green","\tThis team has %d PRIMARY Etracks in Non-CLOSED state\n"), primary_active_count
