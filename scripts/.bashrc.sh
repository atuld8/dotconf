...
EOF
    ...
    if [[ -z "$log_file" ]]; then
        ...
        if [[ -n "$log_base" ]]; then
            log_base="${log_base%/}"
            if [[ -e "$log_base" && ! -d "$log_base" ]]; then
                printf 'logcmd: log base is not a directory: %s\n' "$log_base" >&2
                return 2
            fi
            mkdir -p -- "$log_base" || {
                printf 'logcmd: cannot create log directory: %s\n' "$log_base" >&2
                return 2
            }
...
tx.run() { tmux display-popup -E -w 80% -h 70% -d "#{pane_current_path}" "bash -ic '$*; echo; read -n 1 -s -r -p \"Press any key to continue...\"'"; }

# Tail logs in popup
tx.logs() { tmux display-popup -E -w 90% -h 80% "tail -f ${1:-/var/log/system.log}"; }

# List tx commands
tx.help() { echo "tx.vp tx.hp tx.vx tx.hx tx.vkp tx.hkp tx.vj tx.hj tx.w tx.wj tx.vman tx.hman tx.vw tx.remote tx.scr tx.popup tx.fzf tx.htop tx.lazygit tx.pad tx.run tx.logs"; }
