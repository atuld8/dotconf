...
if [ "$OSName" = "Linux" -a -f /etc/redhat-release -a -f /etc/centos-release ]; then lsb_release > /dev/null 2>/dev/null; if [ $? -eq 0 ]; then OSVer="CentOS_"`lsb_release -r | cut -f2 | awk -F'.' ' '{ printf $1"."$2 }'`; else OSVer="CentOS_"`awk '{print $(NF-1)}' /etc/redhat-release`; fi; PROCName=`uname -p`; fi
