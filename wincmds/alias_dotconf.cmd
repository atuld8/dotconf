...
@DOSKEY cmd.vimacp=%~dp0cd.cmd %~dp0/.. ^&^& %%GIT_CMD_USED%% commit -a --reuse-message=HEAD ^&^& %%GIT_CMD_USED%% pull origin ^&^& %%GIT_CMD_USED%% push ^&^& %~dp0cd.cmd -
@DOSKEY cmd.vimacmp=%~dp0cd.cmd %~dp0/.. ^&^& %%GIT_CMD_USED%% commit -a -m $* ^&^& %%GIT_CMD_USED%% pull origin ^&^& %%GIT_CMD_USED%% push ^&^& %~dp0cd.cmd -
@DOSKEY cmd.vimdl=%~dp0cd.cmd %~dp0/.. ^&^& %%GIT_CMD_USED%% dl $* ^&^& %~dp0cd.cmd -
@DOSKEY cmd.bundleupdate=@FOR /f %%d in ('dir /b') do ( start /b cmd /c "echo %%d & cd %%d & git reset --hard & git pull & cd .." )
...
@DOSKEY set.e.gvimsrv=SET CFG_EDITOR=gvim --servername CFG_EDITOR --remote-silent
@DOSKEY set.e.gvimsrvtab=SET CFG_EDITOR=gvim --servername CFG_EDITOR --remote-tab-silent
