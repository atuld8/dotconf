@REM SETX PROMPT $C$D$S$T$F$S$M[^%COMPUTERNAME^%]$S$P$S%EXTRA_PROMPT_DATA%$_$+$G$G$S >NUL
...
@DOSKEY @winkit.v1=%~dp0cd.cmd "%ProgramFiles%\Windows Kits\10\Debuggers\x64" ^&^& $* ^& %~dp0cd.cmd -
@DOSKEY @winkit.v2=%~dp0cd.cmd "%ProgramFiles%\Windows Kits\8.1\Debuggers\x64" ^&^& $* ^& %~dp0cd.cmd -

@DOSKEY .windbg64.v0="%SYSTEMDRIVE%\Program Files (x86)\Debugging Tools for Windows (x86)\windbg.exe" $*
@DOSKEY .windbg64.v1="%SYSTEMDRIVE%\Program Files (x86)\Windows Kits\10\Debuggers\x64\windbg.exe" $*
@DOSKEY .windbg64.v2="%SYSTEMDRIVE%\Program Files (x86)\Windows Kits\8.1\Debuggers\x64\windbg.exe" $*
