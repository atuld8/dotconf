...
@DOSKEY nbuver=(@FOR /F "tokens=1,* delims=REG_SZ " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion 2^^^>NUL^^^|findstr "BuildID"') DO @ECHO %%a                %%b)^&(@FOR /F "tokens=1,* delims=REG_SZ " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion 2^^^>NUL^^^|findstr "FullVersion"') DO @ECHO %%a            %%b)^&(@FOR /F "tokens=1,2,3,* delims= " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion 2^^^>NUL^^^|findstr "Install\ Type"') DO @ECHO %%a %%b           %%d)^&(@FOR /F "tokens=1,2,* " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config 2^^^>NUL^^^|findstr "EMMSERVER"') DO @echo Master                 %%c)^&(@FOR /F "tokens=1,2,* " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config /v Server /se " " 2^^^>NUL^^^|findstr "Server"') DO @echo %%a                 %%c)^&(@FOR /F "tokens=1,2,* " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config 2^^^>NUL^^^|findstr "Client_Name"') DO @ECHO %%a            %%c)^&(@FOR /F "tokens=1,2,* " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config 2^^^>NUL^^^|findstr "Cluster_Name"') DO @ECHO %%a           %%c)
