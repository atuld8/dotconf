...
@DOSKEY cat.nb.srvconf=type "%%NBU_CONF_PATH%%"NetBackupDB\CONF\server.conf $*
...
@DOSKEY cd.nb.odb=%~dp0cd.cmd "%%NBU_INST_PATH%%"NetBackupDB\WIN64\$*
...
@DOSKEY cmd.nb.md.allplats1=mkdir $*/AMD64 $*/solaris $*/solaris_x86 $*/linuxR_x86 $*/linuxS_x86
...
@DOSKEY r.nb.path=reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion /v INSTALLDIR
...
@DOSKEY nbuver=(@FOR /F "tokens=1,* delims=REG_SZ " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion 2^^^>NUL^^^|findstr "BuildID"') DO @ECHO %%a                %%b)^&(@FOR /F "tokens=1,* delims=REG_SZ " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion 2^^^>NUL^^^|findstr "FullVersion"') DO @ECHO %%a            %%b)^&(@FOR /F "tokens=1,2,3,* delims= " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion 2^^^>NUL^^^|findstr "Install\ Type"') DO @ECHO %%a %%b           %%d)^&(@FOR /F "tokens=1,2,* " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config 2^^^>NUL^^^|findstr "Server"') DO @ECHO %%a                 %%c)^&(@FOR /F "tokens=1,2,* " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config 2^^^>NUL^^^|findstr "Client_Name"') DO @ECHO %%a            %%c)^&(@FOR /F "tokens=1,2,* " %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config 2^^^>NUL^^^|findstr "Cluster_Name"') DO @ECHO %%a           %%c)
@DOSKEY cmd.testOps="%BUILD_DIR%\..\dest\AMD64\OpsCenter\setup.exe" -Debug -NoInstall $*
...
@DOSKEY cp.mstr=@for /f "Tokens=3" %%a in ('reg query HKEY_LOCAL_MACHINE\SOFTWARE\Veritas\NetBackup\CurrentVersion\config /v EMMSERVER') do @echo^|set /p=%%a^|clip
@DOSKEY cp.nb.bin=echo^|set /p=%%nbin%%^|clip
@DOSKEY cp.nb.logs=echo^|set /p=%%NBU_INST_PATH%%NetBackup\logs^|clip
@DOSKEY cp.nb.win.path=echo @SET PATH=%%PATH%%;C:\Program Files\Veritas\NetBackup\bin;C:\Program Files\Veritas\NetBackup\bin\admincmd;C:\Program Files\Veritas\NetBackup\bin\goodies;C:\Program Files\Veritas\NetBackup\sec\at\bin;C:\Program Files\Veritas\NetBackup\bin\support;^|clip
